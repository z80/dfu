 ...
 *   Copyright (C) 2010 by Martin Thomas (timeout-handling)                *
...
void dbg_write(unsigned long dcc_data)
{
	...
	while (len--)
	{
		countdown = 1000000; /* "magic" may have to be changed for other target/frequencies */
		/* wait for data ready */
		while ( (NVIC_DBG_DATA_R & BUSY) && (countdown--) );
