...
PARTITION VolToPart[] = {
	{0, 0},	/* Logical Drive 0 <== Physical Drive 0 */
	{1, 0},	/* Logical Drive 1 <== Physical Drive 1 */
	{2, 0},	/* Logical Drive 2 <== Physical Drive 2 */
	{3, 1},	/* Logical Drive 3 <== Physical Drive 3, 1st Partition */
	{3, 2},	/* Logical Drive 4 <== Physical Drive 3, 2nd Partition */
	{3, 3},	/* Logical Drive 5 <== Physical Drive 3, 3rd Partition */
	{4, 0},	/* Logical Drive 6 <== Physical Drive 4 */
	{5, 0}	/* Logical Drive 7 <== Physical Drive 5 */
};
#endif
...
int _tmain (int argc, TCHAR *argv[])
{
	...
	if (!assign_drives(PHY_DRV_START, PHY_DRV_END)) {
		_tprintf(_T("\nCould not access physical drive.\nPerhaps, you don't have a permission to access physical layer.\n"));
		Sleep(3000);
		return 2;
	}
...
#else
					512, fs->csize, (DWORD)fs->csize * 512);
#endif
			...
			case 'p' :	/* fp <pd#> <size1> <size2> <size3> <size4> - Create partition table */
				{
					...
					xatoi(&ptr, &pts[2]);
					xatoi(&ptr, &pts[3]);
					_tprintf(_T("The drive will be re-partitioned. Are you sure? (Y/n)="));
					_fgetts(ptr, 256, stdin);
					if (*ptr != 'Y') break;
